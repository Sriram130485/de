...
export default function DriverScreen({ navigation }) {
    ...
    // Google Places Autocomplete State (Driver Screen)
    ...
    const handleSelectLocation = async (item) => {
        ...
        // Fetch details if needed (e.g. to get precise lat/lng or full address)
        try {
            // For now, just setting the city text is enough as per current logic
    ...
    const fetchData = async (isRefresh = false) => {
        try {
            ...
            const [jobsRes, requestsRes] = await Promise.all([
                getTrips({ type: "job-board", userId: user._id }),
                getDriverRequests(user._id)
            ]);

            setTrips(jobsRes.trips || []);
    ...
    const handleRequestTrip = async (trip) => {
        try {
            setRequesting(prev => ({ ...prev, [trip._id]: true }));
            ...
            Alert.alert("Success", "Trip request sent! The owner will review it.");
            fetchData(true); // Refresh to update UI with new request status
        } catch (error) {
            Alert.alert("Request Failed", error.message || "Could not send request.");
        } finally {
            setRequesting(prev => ({ ...prev, [trip._id]: false }));
        }
    };
    ...
    const sortedTrips = [...filteredTrips].sort((a, b) => {
        if (appliedSort === 'price_low') {
            return (a.driverPaymentAmount || 0) - (b.driverPaymentAmount || 0);
        }
        if (appliedSort === 'price_high') {
            return (b.driverPaymentAmount || 0) - (a.driverPaymentAmount || 0);
        }
    ...
    const handleGetCurrentLocation = async (isAuto = false) => {
        try {
            ...
            if (!enabled) {
                ...
                if (!enabled) {
                    setIsManualLocation(true); // Switch to manual mode if disabled or user cancelled
            ...
            if (status !== 'granted') {
                setIsManualLocation(true); // Switch to manual mode if denied
            ...
            if (!isAuto) Alert.alert("Error", "Failed to get location. Please ensure GPS is on.");
        } finally {
            setIsLocating(false);
        }
    };
    ...
    if (driverStatus.approvalStatus === 'pending') {
        return (
            <SafeAreaView style={[styles.container, { backgroundColor: theme.background, justifyContent: 'center', alignItems: 'center' }]}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate('UserDocuments')} style={{ padding: 10 }}>
                    <Text style={{ color: theme.primary, fontWeight: 'bold' }}>View My Documents</Text>
                </TouchableOpacity>
            </SafeAreaView>
        );
    }
    ...
    const renderTrip = ({ item }) => {
        ...
        if (existingRequest) {
            ...
            // Using MyTrips style for status text if possible, or keeping badge but cleaner
            statusBadge = (
        ...
        return (
            <TouchableOpacity
                onPress={() => navigation.navigate("TripDetails", { trip: item, existingRequest })}
                style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border, borderWidth: 1 }]}
            >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View style={{ flex: 1, marginRight: 10 }}>

                        {/* Header: Payment Amount as the "Status" equivalent highlight */}
                        <View style={[styles.cardHeader, { justifyContent: 'space-between' }]}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Ionicons name="wallet-outline" size={14} color={theme.success} />
                                <Text style={{ color: theme.success, fontSize: 12, fontWeight: 'bold', marginLeft: 4 }}>
                                    ₹{item.driverPaymentAmount || 'N/A'}
                                </Text>
                            </View>
                        </View>
                    ...
                    {/* Owner/Vehicle Image */}
                    <Image
                        source={{ uri: item.owner?.profileImage || "https://via.placeholder.com/100" }}
                ...
                <View style={{ marginTop: 15, alignItems: 'flex-end' }}>
                    {existingRequest ? (
                        ...
                        <TouchableOpacity
                            onPress={() => navigation.navigate("TripDetails", { trip: item, existingRequest })}
                            style={[styles.actionBtn, { backgroundColor: theme.primary, minWidth: 100, opacity: requesting[item._id] ? 0.7 : 1 }]}
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
            <View style={styles.header}>
                <View>
                    <Text style={[styles.heading, { color: theme.textPrimary }]}>Driver Dashboard</Text>
                    <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Find your next journey</Text>
                </View>
                <TouchableOpacity
                    onPress={() => {
                    ...
                    style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.card, padding: 8, borderRadius: 10, borderWidth: 1, borderColor: theme.border }}
                >
            ...
            <View style={{ padding: 16 }}>
                <View style={[styles.filterCard, { backgroundColor: theme.card, borderColor: theme.border, borderWidth: 1 }]}>
                    ...
                    {/* Location Section */}
                    {!isManualLocation && currentCity ? (
                        <TouchableOpacity
                            onPress={() => setIsManualLocation(true)}
                            style={[styles.locationHeader, { backgroundColor: theme.primary }]}
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                <View style={{ marginLeft: 12, flex: 1 }}>
                                    ...
                                    <Text style={styles.locationFullText} numberOfLines={1}>{currentFullAddress}</Text>
                                </View>
                            </View>
                        </TouchableOpacity>
                    ) : (
                        <View style={{ padding: 15 }}>
                            <Text style={{ color: theme.textSecondary, fontSize: 13, marginBottom: 8, fontWeight: 'bold' }}>
                                Location search
                            </Text>
                            <View style={[styles.filterRow, { height: 50, backgroundColor: theme.secondary, borderRadius: 12, paddingHorizontal: 12 }]}>
                                <Ionicons name="search-outline" size={20} color={theme.textSecondary} style={styles.filterIcon} />
                                <TextInput
                                    ...
                                    onFocus={() => {
                                        if (filterFrom.length >= 4 && citySuggestions.length > 0) setShowCitySuggestions(true);
                                    }}
                                />
                            </View>
                            ...
                            >
                                {isLocating ? (
                                    ...
                                    <>
                                        ...
                                        <Text style={{ color: theme.primary, fontWeight: 'bold', marginLeft: 8 }}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <Text style={[styles.emptyText, { color: theme.textSecondary }]}>
                            {filterFrom || filterTo ? "No trips found matching your research." : "No driving jobs available at the moment."}
                        </Text>
                    }
                />
            )}
...
const styles = StyleSheet.create({
    ...
    // Card Styles adapted from MyTripsScreen
    ...
    primaryBtn: { paddingVertical: 12, paddingHorizontal: 24, borderRadius: 12, alignItems: 'center' }, // Kept for other screens if needed
